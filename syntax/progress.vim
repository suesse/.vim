...
"			Dale Suesse             <dalesuesse@gmail.com>
" Last Change:		Wed Mar 08 11:26:41 EST 2011
...
" Added: Function/Procedure block - currently being ignored
syn region      ProgressFunctionBlock   transparent matchgroup=ProgressFunction start="\<function\>|\<procedure\>" matchgroup=ProgressFunction end="\<end\>|\<forward\>" contains=ALLBUT,ProgressProcedure,ProgressFunction
...
" Removed: where - changed to a reserved word
syn keyword	ProgressFor		each
...
syn keyword	ProgressReserved	authorization auto-ret[urn] back[ground] before-h[ide] begins bell between blank break btos by call can-do can-find
...
syn keyword	ProgressReserved	release reposition retain retry return[s] return-val[ue] revert revoke run save schema screen screen-io screen-lines
...
syn keyword	ProgressReserved	use-index use-revvideo use-underline user user[id] using v6frame value values variable view view-as vms wait-for web-con[text] where
...
syn keyword	ProgressOperator	attach[ment] auto-end-key auto-endkey auto-go auto-ind[ent] auto-resize auto-z[ap] avail[able] available-formats ave[rage] avg backward[s] base-key batch[-mode] bgc[olor] binary
...
syn keyword	ProgressOperator	grid-unit-width-pixe[ls] grid-visible gt height height[-chars] height-p[ixels] help-con[text] helpfile-n[ame] hidden hint hori[zontal] hwnd image image-down
...
syn keyword	ProgressOperator	max-width[-chars] max-width-p[ixels] maximize max[imum] memptr memory menu menu-bar menu-item menu-k[ey] menu-m[ouse] menubar message-area message-area-font message-line
...
" Added: decimal int64 handle datetime
" Removed: format
syn keyword	ProgressType		blob char[acter] class clob com-handle da[te] datetime datetime-tz dec[imal] handle
syn keyword	ProgressType		int64 int[eger] log[ical] longchar memptr raw recid rowid widget-handle var[iable]
